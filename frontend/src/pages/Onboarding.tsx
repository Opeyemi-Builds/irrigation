      ...
      {steps.map((s, i) => {
        ...
        return (
          <React.Fragment key={s.key}>
            ...
            {i < steps.length - 1 && (
              <div style={{ width: '80px', height: '2px', marginBottom: '18px', background: done ? 'var(--accent-primary)' : 'var(--border)', transition: 'background 0.3s' }} />
            )}
          </React.Fragment>
        );
      })}
...
const DeviceStep: React.FC<{ onNext: (data: { farmName: string; productId: string }) => void; onBack: () => void }> = ({ onNext, onBack }) => {
  ...
  const submit = () => {
    ...
    if (productId.trim().length < 4) { setError('Enter the Product ID printed on your device.'); return; }
    setError('');
    onNext({ farmName: farmName.trim(), productId: productId.trim() });
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div>
        ...
        <input
          ...
          style={{ ...inputStyle(focused === 'pid'), fontFamily: 'var(--font-display)', letterSpacing: '2px', fontSize: '18px' }}
...
// ── Step 3: Crop ─────────────────────────────────────────────────────────────
const CropStep: React.FC<{ onSubmit: (data: { crop: string; stage: string; soil: string }) => void; onBack: () => void }> = ({ onSubmit, onBack }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        ...
        <button onClick={() => canSubmit && onSubmit({ crop, stage, soil })} disabled={!canSubmit} style={{
          flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px',
...
const Onboarding: React.FC<Props> = ({ onComplete, onBack }) => {
  ...
  const titles: Record<Step, { title: string; sub: string }> = {
    ...
    crop: { title: 'Set your crop profile', sub: 'Help the advisor give you crop-specific guidance.' },
    done: { title: '', sub: '' },
  };

  const finish = (data: { crop: string; stage: string; soil: string }) => {
    saveFarmProfile({
      farmName,
      crop: data.crop,
